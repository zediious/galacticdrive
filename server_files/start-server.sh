...
if [ -f serverstarter-2.4.1.jar ]; then
    ...
    export URL="https://github.com/ShadowRaptorMC/ServerStarter/releases/download/v2.4.1/serverstarter-2.4.1.jar"
fi
echo $URL
if command -v wget >>/dev/null; then
    echo "DEBUG: (wget) Downloading ${URL}"
    wget -O serverstarter-2.4.1.jar "${URL}"
else
    if command -v curl >>/dev/null; then
        echo "DEBUG: (curl) Downloading ${URL}"
        curl -L -o serverstarter-2.4.1.jar "${URL}"
    else
